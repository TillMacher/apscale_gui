...
def plot_reads_processing(file):
    ...
    if '3_PE merging' in sheet_names:
        ...
        fig = go.Figure(data=[
            go.Bar(name='Merged', x=x1_values, y=y_values, orientation='h', marker_color='Blue'),
    ...
    if '4_primer_trimming' in sheet_names:
        ...
        fig = go.Figure(data=[
            go.Bar(name='Trimmed', x=x1_values, y=y_values, orientation='h', marker_color='Blue'),
            go.Bar(name='Discarded', x=x2_values, y=y_values, orientation='h', marker_color='Orange')])
        fig.update_layout(barmode='stack', template='plotly_white', width=1000, height=1000)
        fig.update_xaxes(title='reads (%)')
        fig.show()
    ...
    if '5_quality_filtering' in sheet_names:
        ...
        fig = go.Figure(data=[
            go.Bar(name='Passed', x=x1_values, y=y_values, orientation='h', marker_color='Blue'),
            go.Bar(name='Discarded', x=x2_values, y=y_values, orientation='h', marker_color='Orange')])
        fig.update_layout(barmode='stack', template='plotly_white', width=1000, height=1000)
        fig.update_xaxes(title='reads (%)')
        fig.show()
    ...
    if '6_dereplication' in sheet_names:
        ...
        fig = go.Figure(data=[
            go.Bar(name='Unique', x=x1_values, y=y_values, orientation='h', marker_color='Blue'),
